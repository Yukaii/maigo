...
pub const OAuthServer = struct {
    ...
    fn storeAuthorizationCode(self: *OAuthServer, auth_code: AuthorizationCode) !void {
        try self.db.insertAuthorizationCode(auth_code.code, auth_code.client_id, auth_code.user_id, auth_code.redirect_uri, auth_code.expires_at);
    }
    ...
    fn storeAccessToken(self: *OAuthServer, token: AccessToken) !void {
        try self.db.insertAccessToken(token.token, token.client_id, token.user_id, token.scope, token.expires_at, token.refresh_token);
    }
...
test "oauth token expiration" {
    ...
    try db.insertAccessToken(expired_token, client.id, user_id, "url:read url:write", past_timestamp, null);
    ...
    try db.insertAccessToken(valid_token, client.id, user_id, "url:read url:write", future_timestamp, null);
